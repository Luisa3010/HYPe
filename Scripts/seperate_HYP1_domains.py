##### use a fuzzy matching approach to get the HVD from a G. Pallida HYP1 read
...
def find_close_matches(text, target = "YERGGG"):
...
def cut_sequence(seq):
    ...
    try:
        # find index of first YERGGG
        start, end = find_close_matches(translated_seq,"YERGGG")
        # cut DNA there
        seq = seq[start*3:end*3 + 6*3]
    except:
        print("Warning: Couldn't cut a sequence")
    return seq

sequences = read_fasta("/home/luisa/Documents/Work/Uni Cambridge/Data/HYP1_sanger/HYP1_nucleotide_fasta")
cut_sequences = {}
